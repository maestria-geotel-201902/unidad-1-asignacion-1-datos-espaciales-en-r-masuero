...
## Reading layer `PROVCenso2010' from data source `/home/masue/unidad-1-asignacion-1-datos-espaciales-en-r-masuero/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
plot(prov[prov$PROV %in% '26', 'PROV'])
```
...
## 26   26  04 SANTIAGO RODRÍGUEZ   0426 MULTIPOLYGON (((270283.9 21...
plot(miprov)
```
...
## proj4string:    +proj=utm +zone=19 +datum=WGS84 +units=m +no_defs
##   PROV REG          TOPONIMIA ENLACE                           geom
## 1   26  04 SANTIAGO RODRÍGUEZ   0426 MULTIPOLYGON (((270283.9 21...
##           areaenm2
## 1 1148163890 [m^2]
prov %>% dplyr::filter(PROV %in% '26') %>% dplyr::select(areaenm2) %>% plot
```
...
``` r
prov %>% dplyr::filter(PROV %in% c('26', '12', '20')) %>% dplyr::select(areaenm2) %>% plot
```
...
## Reading layer `MUNCenso2010' from data source `/home/masue/unidad-1-asignacion-1-datos-espaciales-en-r-masuero/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
mimun <- mun %>% dplyr::filter(PROV %in% '26')
mimun %>% plot
```
